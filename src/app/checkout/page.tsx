...
import { createBooking } from '@/lib/services/db';
import { generateId } from '@/lib/id-generator';
import AuthModal from '@/components/auth/AuthModal';
import {
  User, Phone, Mail, MapPin, Calendar, Clock, CreditCard,
  ArrowLeft, ArrowRight, Check, Shield, ChevronRight, LogIn
} from 'lucide-react';
import Link from 'next/link';

// Default tenant ID — same as admin panel
const TENANT_ID = 'demo_lab_001';
...
function CheckoutContent() {
  ...
  const handlePayment = async () => {
    ...
    setProcessing(true);
    try {
      // Generate a proper booking ID using the admin's ID system
      const bookingId = await generateId(TENANT_ID, 'LAB');

      // Create booking in the NEW format that shows up in the admin panel
      const bookingData = {
        tenantId: TENANT_ID,
        bookingId,
        patientId: user.uid,
        ...
        collectionType: 'home_collection' as const,
        date: form.date,
      ...
      const firestoreId = await createBooking(bookingData as any);

      clearCart();
      router.push(`/booking/confirmation/${firestoreId}?name=${encodeURIComponent(form.name)}&total=${state.totalPrice}&bookingId=${encodeURIComponent(bookingId)}`);
    } catch (err) {
      ...
      <>
        ...
        <div className="min-h-screen flex items-center justify-center bg-surface/30">
          <div className="text-center max-w-md mx-auto px-4">
            ...
            <h1 className="text-2xl font-bold text-primary mb-2">Login Required</h1>
            <p className="text-text-muted mb-6">
              Please sign in to proceed with your booking. This helps us track your bookings, send reports, and provide a better experience.
            </p>
            <button
              onClick={() => setShowAuthModal(true)}
              className="btn-primary w-full justify-center py-3 rounded-xl text-base mb-3"
            >
              <LogIn size={18} /> Sign In / Create Account
            </button>
            <Link href="/cart" className="text-sm text-text-muted hover:text-accent transition-colors">
              ← Back to Cart
            </Link>
          </div>
        </div>
              ...
              <div className="sticky top-[180px] bg-white rounded-2xl border border-border/50 p-5 shadow-sm">
                ...
                {user && (
                  <div className="mt-4 pt-3 border-t border-border/50">
                    <p className="text-xs text-text-muted">Booking as</p>
                    <p className="text-sm font-semibold text-primary">{user.displayName || user.email}</p>
                  </div>
                )}
              </div>
            </div>
